...
| 角色 | 前台暴露 | 有/无状态 | 扩缩特性 |
| --- | --- | --- | --- |
| `standalone` | HTTP（axum）+ gRPC（tonic），并在进程内运行全部内部职责 | 含状态（内含 Ingester） | 评估用单实例；不适合作为横向扩展单元 |
...
| `querier` | 设计上为 Arrow Flight 分布式扫描端 | 无状态 | 可水平扩 |
| `compactor` | 后台 tick 循环（预拉起），角色本体空转 | 无状态（逐 tick 处理） | **建议单实例**（见高可用一节） |
| `alert_manager` | 后台评估 + 派发两条循环（预拉起），角色本体空转 | 有状态（评估状态、事件状态在进程内） | 建议单实例 |

<Warning>
**Querier 角色当前为占位 / 未完成。** 角色分发函数目前只记录一条 "querier role started (stub)" 日志并空转，独立的 Arrow Flight 查询服务端实现尚未接线。分布式查询的散播协议（见数据流章节）在代码中存在并被协调端调用，但作为独立 `querier` 进程对外提供 Flight `do_get` 的那一层尚未完成。当前分布式查询的可用形态是 `standalone` 进程内的引擎组合。
</Warning>
...
<Note>
**compactor 与 alert_manager 的循环属于"预拉起"模式：** 它们在统一装配阶段就以独立后台任务运行，而 `compactor::run()` / `alert_manager::run()` 这两个角色本体函数只是空转占位。因此把进程的 `[node].roles` 设为 `compactor` 或 `alert_manager`，效果是"该进程的角色本体空转 + 后台循环照常跑"。
</Note>

## 如何选择角色

角色通过 `[node].roles` 配置；可用环境变量覆盖。环境变量统一前缀 `MS_`，**section 与 field 之间用 `.`（点）分隔**（由 figment `Env::prefixed("MS_").split(".")` 解析）。例如 `MS_NODE.ROLES`、`MS_STORE.META.DSN`、`MS_HTTP.PORT`。
...
入口经 Router（限流 + 一致性哈希）落到某个 Ingester；Ingester 先写 WAL（落盘持久），再写内存 Arrow 缓冲；后台 flush 循环按时间窗或大小阈值把缓冲编码成 Parquet + Tantivy 索引上传对象存储，最后把 FileMeta 落 Postgres，并截断已 flush 的 WAL 段。

```
                 X-Org-Id            FNV-1a( org_id|stream )
 client ──HTTP/gRPC──► Router ──一致性哈希──► Ingester (有状态)
 :5080/:5082          (无状态,限流)            │
                       │ 429 + Retry-After     │ 1) 写 WAL（落盘, 可配 fsync 策略）
                       │ 超过 org QPS 时        │ 2) 写内存 Arrow 缓冲（按 seq 累积）
                                               ▼
                              后台 flush 循环（flush_interval_secs / 缓冲阈值触发）
                                               │
                          snapshot → Parquet(SNAPPY) + Tantivy 侧车
                                               │ 上传
                          ┌────────────────────┴────────────────────┐
                          ▼                                          ▼
                  对象存储（local / s3 / azure / gcs）        Postgres：FileMeta
                  {org}/{type}/{stream}/{date}/{ksuid}.parquet  (time_range, rows,
                  …{ksuid}.parquet.tantivy.tar.zst              min/max, object_key)
                                               │
...
查询入口在暴露 HTTP 的节点上。引擎按集群规模逐层包装：本地 DataFusion 引擎 →（≥2 个 querier 对等节点时）分布式引擎 →（指定了远程集群时）联邦引擎。
...
```
 client ──POST /api/v1/query──► QueryService
                       ...
                       同步执行            异步 → 建 search_jobs 行，返回 202 + job_id
                          │
       引擎组合：DataFusion(本地) → Distributed(≥2 querier) → Federated(远程集群)
                          │
            FileMeta 分区裁剪（org/stream/stream_type/time_range）+ Tantivy MATCH 裁剪
                          │
      分布式分片：fxhash(object_key) % peer_count → 每片编码为 QueryShard(Ticket)
                          │  Arrow Flight do_get（:5082）
            ┌─────────────┼─────────────┐
            ▼             ▼             ▼
        Querier 对等   Querier 对等   …（各自读 Parquet, SELECT * 扫描, 回传 RecordBatch）
            └─────────────┼─────────────┘
                          ▼
        协调端 UNION ALL → 注册 MemTable → 执行完整用户 SQL（聚合/过滤/投影）
                          ▼
                   QueryResult（columns, rows, scanned_rows, took_ms）
```

<Note>
分布式查询的分片用 `fxhash(object_key)` 散到对等节点；分片 SQL 仅做扫描（`SELECT * FROM <stream>`），完整聚合在协调端执行以避免部分/最终聚合不一致。仅当集群中有 ≥2 个 querier 对等节点时才走分布式；否则回退到本地引擎，无网络跳。
</Note>
...
通过 `?clusters=local,sf,nyc` 指定目标集群。协调端本地扫描后，对每个启用的远程集群发起一次 Arrow Flight `do_get`（带 Bearer token），把各集群回传的批次与本地 UNION ALL 后执行完整 SQL。
...
<Steps>
  <Step title="节点注册">
    心跳任务周期性 upsert `(node_id, role, advertise_addr, last_heartbeat_at_micros)` 到 Postgres `cluster_nodes` 表（主键 `node_id`，`ON CONFLICT DO UPDATE`）。首次心跳立即发出，之后按间隔。
  </Step>
  <Step title="心跳间隔">
  ...
  <Step title="选址算法">
    Router 选 Ingester：对 `org_id|stream_name` 做 FNV-1a 一致性哈希后取模，确定性落到某个 Ingester。Router 选 Querier：朴素轮询（`now_ns % peer_count`），不是完整一致性哈希。分布式查询分片：对 `object_key` 做 `fxhash` 取模散到 querier 对等节点。
  </Step>
  <Step title="Arrow Flight">
    协调端把 `QueryShard`（含 org/stream/sql/file_metas/time_range）prost 编码进 `Ticket`，经 Flight `do_get` 发到对等节点；对等节点读 Parquet、注册 MemTable、跑分片 SQL、回传 `FlightData` 流。Flight 与 gRPC NodeService、IngestGrpc 共用同一端口（默认 5082）。
  </Step>
</Steps>

<Info>
集群内部的本地 Flight 调用不带鉴权；只有联邦 / 远程集群调用使用可选的 Bearer token。
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="对象存储" icon="cloud">
    Parquet + Tantivy 侧车。`[store.object].backend`：`local`（默认，`root=./data/objects`）/ `s3`（含 MinIO、R2、阿里云 OSS，用 `endpoint` 覆盖）/ `azure` / `gcs`。凭据优先级：环境变量 > 凭据文件 > 内联 TOML。
  </Card>
  <Card title="无外部缓存 / 共识" icon="memory">
    仅进程内 moka LRU+TTL 缓存、governor 限流、tokio 运行时。无 Redis / Memcached，无外部共识组件。
  </Card>
...
| 端口 | 协议 / 服务 | 配置键 | 说明 |
| --- | --- | --- | --- |
| 5080 | HTTP（axum） | `[http].bind`（默认 `0.0.0.0`）、`[http].port` | `/api/v1/*`、`/metrics`、`/healthz`、`/readyz`、`/.well-known/acme-challenge/` 等 |
| 5082 | gRPC（tonic） | `[grpc].bind`、`[grpc].port`、`max_message_size_mb`（默认 32MB） | IngestGrpc + Arrow Flight `do_get` + Cluster 心跳，三服务共用 |
| 80 | TLS 模式下的明文 HTTP | `[http.tls].plain_port` | 健康检查 + ACME HTTP-01 挑战 + 301 跳转 HTTPS |
| 443 | TLS 模式下的 HTTPS（rustls + SNI） | `[http.tls].port` | 完整路由；需 `domain-acme-tls` feature |
...
<Tabs>
  ...
  <Tab title="Kubernetes">
    ...
    | `50-querier.yaml` | Deployment | 2 | `querier` | 无状态；5080 + 5082（Arrow Flight）；ClusterIP；就绪探针 5s |
...
<CardGroup cols={2}>
  ...
  <Card title="Querier — 可水平扩" icon="magnifying-glass">
    无状态。≥2 个对等节点才触发分布式扫描。注意：独立 querier 的 Flight 服务端当前为占位，分布式查询的完整落地形态以 standalone 内引擎为准。瓶颈在 Parquet 读取 + DataFusion 内存。
  </Card>
...
<Warning>
落地前请明确以下"现状"边界：`querier` 独立角色为占位（分布式 Flight 服务端未完成）；多角色数组仅首元素生效；ACME 签发/续期为占位且未拉起；分布式共识 WAL term source 仍为静态占位（多节点共识未实现）；联邦查询与 SSO（仅 OIDC，SAML 未实现）等为 license/feature 门控。
</Warning>
