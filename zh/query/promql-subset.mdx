...
| 功能 | 状态 | 说明 |
|---|---|---|
| native-histogram 函数 `histogram_count` / `histogram_sum` / `histogram_avg` / `histogram_fraction` / `histogram_stddev` / `histogram_stdvar` | ❌ → 400 | classic `le` bucket 模型下不适用；分位数走 `histogram_quantile` |
| 二元 `default` 填充 | ❌ → 400 | — |
| range 查询里的子查询 | ⚠️ | 回退到 instant 求值（不做外层步进 × 内层步进的双重扫描）；instant / 告警路径精确 |
| `timestamp()` | ⚠️ | instant 路径返回求值时刻；range 路径返回各点自身时间戳 |
...
<Note>
  recording / alerting rules **不走 PromQL**，而是 MoleSignal 自己的 `AlertRule` 模型，
  与 Prometheus rule format 不互通。见 [告警](/zh/alerting)。
</Note>
