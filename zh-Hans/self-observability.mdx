...
MoleSignal 可以将自身遥测写入受保护的 `_sys` 组织。每类信号都使用准确名称 `_molesignal`，并通过数据流类型区分。

| 信号 | 类型化数据流 | 内容 |
|---|---|---|
| 日志 | `logs/_molesignal` | 结构化进程事件与链路上下文。 |
...
在角色拆分部署中，非 ingester 节点会将自身遥测转发给健康的 ingester。请在所有角色上设置相同的 `MS_SELF_TELEMETRY_CLUSTER_TOKEN`。

## 后端分布式链路追踪

MoleSignal 会为 HTTP、gRPC、Flight SQL、数据库、对象存储、采集、查询、后台 Worker 与 Intelligence 阶段创建链路。
...
采样器和自身采集/导出队列都有容量上限，并对业务流量保持故障开放。你可以在 `/metrics` 查看固定基数指标，并通过 `GET /api/v1/system/telemetry` 查看详细健康状态。
