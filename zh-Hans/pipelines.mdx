...
<Note>
  ...
  步骤挂一个[流水线函数](/zh-Hans/ingestion#pipeline-functions)。
</Note>

<Frame caption="调度流水线">
  <img src="/images/diagrams/pipeline.zh-Hans.light.svg" alt="调度流水线" className="block dark:hidden" />
  <img src="/images/diagrams/pipeline.zh-Hans.dark.svg" alt="调度流水线" className="hidden dark:block" />
</Frame>
...
每个转换是一段 [VRL](/zh-Hans/ingestion#pipeline-functions) 脚本。转换检视面板的「复用」下拉里，可选用
**内置预设**或任意已保存的[函数](/zh-Hans/ingestion#pipeline-functions)——选中即把脚本灌入该步骤，之后
可内联编辑。
...
**扩展表**是一张 key → 记录的查找表，可在转换里 join。在 **函数 → 扩展表** 新建，添加行（一个 key 加若
干命名字段），再在 VRL 里查找：
...
当你把一个兜底源数据流扇出成每服务（或每租户）目标流——例如用上面的 `route-by-service` preset——源数据流
