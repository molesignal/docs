...
MoleSignal 是一个**单一二进制**，服务所有角色；可自行选择某个进程运行哪些角色。这让它既能当作
一条命令的沙箱，也能横向扩展成集群。
...
<Tabs>
  <Tab title="Docker Compose">
    ...
    <Warning>
      `connector` 不是当前运行角色。如果你的 Checkout 仍包含旧
      `molesignal-connector` 服务，请将其移除；Connector Runner 由 `alert_manager` 持有。
    </Warning>
  </Tab>
...
- **单一二进制**，所有角色同一镜像。
- **Prometheus `/metrics`**，包含固定基数的缓存、对象存储、采集、查询、告警与自身可观测性指标。
