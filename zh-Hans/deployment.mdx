...
| `router` | 反向代理 + 限流 | 无状态 |
| `intake` | 数据接入 + 预写日志 + 缓冲 + 周期 flush 到列式文件 | 本地日志（≤ flush 窗口） |
| `querier` | 分布式扫描端 + 查询执行 | 无状态 |
| `compactor` | 周期合并文件 + 清理过期数据 | 无状态 |
| `alert_manager` | 规则评估、升级、报告、RCA 维护与 Connector Runner | 无状态 |

角色由 `[node].roles` 配置（或 `MS_NODE.ROLES`）选择。只有 intake 持有本地状态（flush 窗口内的
WAL），因此其他角色都能自由扩缩。
...
<Tabs>
  <Tab title="Docker Compose">
    ...
    `4317`。如需从宿主机发送 OTLP gRPC，请给 standalone 或 intake 服务增加
  ...
  <Tab title="Kubernetes">
    ...
    同一镜像服务所有角色；按 Deployment/StatefulSet 设置 `MS_NODE.ROLES`。intake 以 StatefulSet
...
- **Postgres** —— 元数据、IAM、数据流、告警、报告、Agent 资源与集群状态。
