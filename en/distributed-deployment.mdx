...
| --- | --- | --- | --- |
| `standalone` | HTTP (axum) + gRPC (tonic), running every internal responsibility in-process | Stateful (embeds an Ingester) | Single instance for evaluation; not suitable as a horizontal scaling unit |
...
| `querier` | Designed to be the Arrow Flight distributed scan endpoint | Stateless | Scales horizontally |
...
Roles are configured via `[node].roles`; they can be overridden with environment variables. Environment variables share the `MS_` prefix, with **section and field separated by a `.` (dot)** (parsed by figment `Env::prefixed("MS_").split(".")`). For example: `MS_NODE.ROLES`, `MS_STORE.META.DSN`, `MS_HTTP.PORT`.
...
The entry point lands on some Ingester via the Router (rate limiting + consistent hashing); the Ingester first writes the WAL (durably persisted to disk), then writes the in-memory Arrow buffer; a background flush loop encodes the buffer into Parquet + a Tantivy index by time window or size threshold and uploads it to the object store, finally landing the FileMeta in Postgres and truncating the flushed WAL segments.
...
```
 ...
 :5080/:5082          (stateless, rate limit)         │
                       ...
                       │ when over org QPS             │ 2) write in-memory Arrow buffer (accumulated by seq)
                                               ▼
                              background flush loop (triggered by flush_interval_secs / buffer threshold)
                                               │
                          snapshot → Parquet(SNAPPY) + Tantivy sidecar
                                               │ upload
                          ┌────────────────────┴────────────────────┐
                          ▼                                          ▼
...
The query entry point is on a node that exposes HTTP. The engine is wrapped layer by layer according to cluster size: local DataFusion engine → (with ≥2 querier peers) distributed engine → (when a remote cluster is specified) federated engine.
...
```
 client ──POST /api/v1/query──► QueryService
                       ...
                       synchronous execution   async → create a search_jobs row, return 202 + job_id
                          │
       engine composition: DataFusion(local) → Distributed(≥2 querier) → Federated(remote cluster)
                          │
            FileMeta partition pruning (org/stream/stream_type/time_range) + Tantivy MATCH pruning
                          │
      distributed sharding: fxhash(object_key) % peer_count → each shard encoded as a QueryShard(Ticket)
                          │  Arrow Flight do_get (:5082)
            ┌─────────────┼─────────────┐
            ▼             ▼             ▼
        Querier peer   Querier peer   …(each reads Parquet, SELECT * scan, returns RecordBatch)
            └─────────────┼─────────────┘
                          ▼
        coordinator UNION ALL → register MemTable → execute the full user SQL (aggregation/filter/projection)
                          ▼
                   QueryResult (columns, rows, scanned_rows, took_ms)
```

<Note>
Distributed query shards are fanned out to peers via `fxhash(object_key)`; the shard SQL only performs a scan (`SELECT * FROM <stream>`), while the full aggregation runs on the coordinator to avoid partial/final aggregation inconsistencies. The distributed path is taken only when the cluster has ≥2 querier peers; otherwise it falls back to the local engine with no network hop.
</Note>
...
Target clusters are specified via `?clusters=local,sf,nyc`. After scanning locally, the coordinator issues one Arrow Flight `do_get` (with a Bearer token) to each enabled remote cluster, then UNION ALLs the batches each cluster returns with the local data and executes the full SQL.
...
<Steps>
  ...
  <Step title="Placement algorithm">
    Router selecting an Ingester: applies FNV-1a consistent hashing over `org_id|stream_name` then mods, landing deterministically on a particular Ingester. Router selecting a Querier: naive round-robin (`now_ns % peer_count`), not full consistent hashing. Distributed query sharding: `fxhash` over `object_key` then mods, fanning out to querier peers.
  </Step>
  <Step title="Arrow Flight">
    The coordinator prost-encodes a `QueryShard` (containing org/stream/sql/file_metas/time_range) into a `Ticket` and sends it to peers via Flight `do_get`; the peer reads Parquet, registers a MemTable, runs the shard SQL, and returns a `FlightData` stream. Flight shares the same port (default 5082) with the gRPC NodeService and IngestGrpc.
  </Step>
</Steps>

<Info>
Local Flight calls within the cluster are unauthenticated; only federated / remote-cluster calls use an optional Bearer token.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Object store" icon="cloud">
    Parquet + Tantivy sidecars. `[store.object].backend`: `local` (default, `root=./data/objects`) / `s3` (including MinIO, R2, Alibaba Cloud OSS, overridden via `endpoint`) / `azure` / `gcs`. Credential precedence: env vars > credential file > inline TOML.
  </Card>
  <Card title="No external cache / consensus" icon="memory">
    Only in-process moka LRU+TTL caches, the governor rate limiter, and the tokio runtime. No Redis / Memcached, no external consensus component.
  </Card>
...
| --- | --- | --- | --- |
| 5080 | HTTP (axum) | `[http].bind` (default `0.0.0.0`), `[http].port` | `/api/v1/*`, `/metrics`, `/healthz`, `/readyz`, `/.well-known/acme-challenge/`, etc. |
| 5082 | gRPC (tonic) | `[grpc].bind`, `[grpc].port`, `max_message_size_mb` (default 32MB) | IngestGrpc + Arrow Flight `do_get` + Cluster heartbeat, three services sharing one port |
| 80 | Plaintext HTTP in TLS mode | `[http.tls].plain_port` | Health checks + ACME HTTP-01 challenge + 301 redirect to HTTPS |
| 443 | HTTPS in TLS mode (rustls + SNI) | `[http.tls].port` | Full routing; requires the `domain-acme-tls` feature |
...
<Tabs>
  ...
  <Tab title="Kubernetes">
    ...
    | `50-querier.yaml` | Deployment | 2 | `querier` | Stateless; 5080 + 5082 (Arrow Flight); ClusterIP; readiness probe 5s |
...
<CardGroup cols={2}>
  ...
  <Card title="Querier — scales horizontally" icon="magnifying-glass">
    Stateless. Distributed scans only trigger with ≥2 peers. Note: the standalone querier's Flight server is currently a placeholder, so the fully landed form of distributed query is the in-process engine of standalone. The bottleneck is Parquet reads + DataFusion memory.
  </Card>
