...
The list response includes key id, primary state, creation time, and retirement time. It never
...
TOML with `MS_AUTH_JWT_SECRET_OVERRIDE` only when deployment policy requires it.
