...
- a **schema** (inferred and evolved as data arrives),
- a **retention** policy, and
- a **`queryable`** flag (default on) — turn it off to keep ingesting and retaining a stream while
  hiding it from query selectors and rejecting it from search; useful for a catch-all source stream
  that only feeds a routing [pipeline](/en-US/pipelines).
...
[Security](/en-US/security) for details.
...
losing context. See [Cross-signal correlation](/en-US/correlation).
