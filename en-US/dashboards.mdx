...
You can import Grafana JSON; MoleSignal migrates the supported parts into its own dashboard model.

## Panels

Each panel runs an independent query and renders it with a visualization plugin. New dashboards
...
variables, and the time range are mapped to MoleSignal's model. Supported query targets preserve
their SQL or PromQL. Legacy `graph`/`line`, `bar`, and `bargauge` types map to current plugins;
unsupported visualizations fall back to `time_series` and should be reviewed after import.

## Export, report, and share

Create a scheduled [report](/en-US/reports) from a dashboard when you need PDF, PNG, CSV, SVG, or JSON
delivery. Use **Share** for authenticated, cross-workspace, or restricted public access. Public
panels execute only their saved queries and remain subject to workspace sharing policy.
