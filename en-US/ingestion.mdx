...
attached to a pipeline step. The runtime supports:
...
- **JavaScript** — opt-in, built on `deno_core` (V8). It is available only when the binary is built
  with `--features js-runtime`; there is no runtime TOML switch.
