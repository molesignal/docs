...
<Tabs>
  <Tab title="Docker Compose">
    ...
    ingester service. Do not use `5082` as a substitute.

    <Warning>
      `connector` is not a current runtime role. If your checkout still contains a legacy
      `molesignal-connector` service, remove it; connector runners are owned by `alert_manager`.
    </Warning>
  </Tab>
...
- **Postgres** — metadata, IAM, streams, alerts, reports, Intelligence resources, and cluster state.
- **Object store** — `local`, `s3` (and S3-compatible: MinIO, R2, Aliyun OSS), `azure`, or `gcs`.

## Operations

- **Single binary**, same image for all roles.
- **Prometheus `/metrics`** with fixed-cardinality cache, object-store, ingest, query, alerting, and
