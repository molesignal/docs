...
| `intake` | Carries WAL replay + buffering + periodic flush (the flush loop is spawned at startup when the role is configured) | **Stateful** (WAL, in-memory buffer, file_meta cache) | Sharded via consistent hashing; scaling must account for the WAL persistent volume and rebalancing |
...
<Frame caption="Intake path">
  <img src="/images/architecture/intake_en_light.svg" alt="Intake path" className="block dark:hidden" />
  <img src="/images/architecture/intake_en_dark.svg" alt="Intake path" className="hidden dark:block" />
</Frame>
