...
<Note>
**`/metrics`** (GET, Prometheus text 0.0.4) is always mounted. Restrict it at the reverse proxy or network boundary. It exposes fixed-cardinality cache, object-store, WAL, query, alerting, and self-observability metrics.
</Note>
...
| --- | --- | --- | --- | --- |
| Liveness | `GET /api/v1/healthz` | WAL replay complete (Ingester-relevant only; other roles bypass it) **and** the object store is not degraded | replay in progress **or** object store connection lost | Process liveness and dependency health |
...
<Steps>
  ...
  <Step title="Ingester WAL replay">
    At startup the Ingester scans the segment files under `[wal].dir`, replays them into the in-memory buffer by `(org, stream_type, stream)`, forces a single flush once everything is loaded, then sets `replay_done=true`. `/api/v1/readyz` returns 503 until replay completes.
  </Step>
</Steps>
...
<Warning>
Before going live, be clear about the following current boundaries: the distributed-consensus WAL term source is still static (multi-node consensus is not implemented); federated query and OIDC/SAML SSO require their license features; and changing `[node].roles` requires a process restart.
</Warning>
