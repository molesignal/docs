...
MoleSignal can ingest its own telemetry into the protected `_sys` organization. Each signal uses
the exact stream name `_molesignal` with its own stream type.

| Signal | Typed stream | Content |
|---|---|---|
| Logs | `logs/_molesignal` | Structured process events and trace context. |
...
The destination cannot be changed to a tenant organization. Public ingest, stream mutation, and
pipelines cannot create, overwrite, or delete `_molesignal`.

In split-role deployments, non-ingester nodes forward self-telemetry to a healthy ingester. Set the
same `MS_SELF_TELEMETRY_CLUSTER_TOKEN` on every role.

## Backend distributed tracing

MoleSignal instruments HTTP, gRPC, Flight SQL, database, object storage, ingestion, query,
background workers, and Intelligence stages.
...
The sampler and self-ingest/export queues are bounded and fail open for business traffic. Inspect
their fixed-cardinality metrics on `/metrics` and detailed health at
