...
<Note>
  ...
  the ingest hot path instead, attach a [function](/en-US/functions) to
  the ingest step.
</Note>
...
Each transform is a [VRL](/en-US/functions) script. The transform inspector lets
you reuse a **built-in preset** or any saved [function](/en-US/functions) from
...
An **extend table** is a key → record lookup table you can join against from a transform. Open
**Extend tables**, create a table, add rows (a key plus named fields), and look it up from VRL:
...
<Note>
  The schedule runner is a singleton: in a distributed cluster it runs only on the **alert-manager**
...
- `pipelines.read` lists pipelines and reads their run history.
- `pipelines.create` creates a pipeline.
- `pipelines.edit` changes its graph, transforms, schedule, or lookback.
