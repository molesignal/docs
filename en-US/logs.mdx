...
and lets you add filters without rewriting the full query.
...
The query planner applies the current organization boundary even when you submit custom SQL.
...
Save a useful query as a saved view. You can reopen it, run it over another time window, attach it
to a report, or use it as the starting point for an alert.

<Tip>
  Preserve stable dimensions such as `service.name`, `deployment.environment`, `trace_id`, and
  `host.name` during ingestion. They make cross-signal pivots reliable.
</Tip>
...
<Card title="Cross-signal correlation" icon="link" href="/en-US/correlation">
  Follow a log event into its trace, service, metrics, or host context.
</Card>
