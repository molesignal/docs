...
| **Enterprise Edition** | Runs with a verified, active signed license and its entitlements. |

Internal API values can use compatibility identifiers such as `community`, `oss`, or `pro`. The UI
maps them to the edition names above.
...
permission to configure or use it.
...
The snapshot includes verification, expiration, issued-to identity, feature names, ingest limit,
