...
In production, replace the host with the externally reachable URL configured for your deployment.
...
Routes that cannot use a normal organization bearer token validate their own narrow credential or
policy. Current examples include:
...
Do not treat a public route as unrestricted. Its handler validates signup policy, a share token,
...
- Compatibility ingest endpoints use their native payloads, including Protobuf, NDJSON, or
...
Large Intelligence evidence, generated reports, profile blobs, and exports can be stored as files
...
Responses can include `X-Request-Id` and `X-Trace-Id`. Include them when reporting a server-side
failure.
...
<CardGroup cols={2}>
  ...
  <Card title="Ingest recipes" icon="inbox" href="/en-US/api/ingest/native-json">
