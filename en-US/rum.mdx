...
Real User Monitoring (RUM) records browser user experience and connects browser telemetry with
...
**Sessions** lists recent browser sessions. Open a session to inspect the action timeline, replay
...
Upload source maps from **RUM → Source maps** or the upload route. MoleSignal uses uploaded maps to
...
A slow browser action can link directly to the corresponding service trace, logs, and metrics while
...
<CardGroup cols={2}>
  ...
  <Card title="Source maps" icon="file-code" href="/en-US/rum/source-maps">
    Restore minified JavaScript errors to source locations.
  </Card>
